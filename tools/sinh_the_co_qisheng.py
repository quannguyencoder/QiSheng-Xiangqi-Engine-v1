"""
QiSheng - sinh the co tu VAN CO CUA CHINH QISHENG, roi nho Pikafish cham diem.

Vi sao can: toan bo 16 trieu mau hien co deu la the co PIKAFISH tu choi. Nhung
QiSheng danh khac Pikafish, no di vao nhung the co ma Pikafish khong bao gio
...
tu dau: tu van co that cua QiSheng thay vi cua Pikafish.

  python3 tools/sinh_the_co_qisheng.py --output data/data_qisheng_s0.jsonl \\
      --so-van 200 --depth 6
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Sinh the co tu van co QiSheng")
    ...
    # Buoc 1: cho QiSheng tu danh, thu thap the co no di qua
    print(f"Danh {args.so_van} van QiSheng vs QiSheng (depth {args.depth})...",
          flush=True)
    the_co = []
