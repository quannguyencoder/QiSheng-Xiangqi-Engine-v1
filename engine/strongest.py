...
def chuan_bi(duong_mang: str = MANG_MAC_DINH,
    ...
    if _da_chuan_bi:
        return _dung_c
    _da_chuan_bi = True
    if not c_core.co_loi_c() or not os.path.exists(duong_mang):
        return False
    ...
    c_core.tim_kiem_khoi_tao(trong_so, 505.0 - tho)
    _dung_c = True
    return True
...
def tim_nuoc_di_theo_gio(board: Board, side_to_move: str, giay: float = 10.0,
    ...
    if dung_sach:
        ...
        mv = book.tra_sach(board, side_to_move, board_hash(board, side_to_move))
        if mv is not None:
            return 505, mv, 0, 0
    if chuan_bi():
        return c_core.tim_kiem_theo_gio(board, side_to_move, giay)
...
def tim_nuoc_di(board: Board, side_to_move: str, depth: int = 8,
    ...
    if chuan_bi():
        return c_core.tim_kiem(board, side_to_move, depth)
